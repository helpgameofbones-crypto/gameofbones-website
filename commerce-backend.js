...
(() => {
  ...
  async function capture() { if (captureSent || !valid() || !marketingConsent()) return; captureSent = true; try { await window.GOB_API.abandonedCart({ phone: phone(), email: get('[autocomplete="email"]'), name: fullName(), items: items(), total: subtotal() }); } catch (_) { /* Do not retry every keystroke after a server-side rate limit. */ } }
  form.addEventListener('input', () => { clearTimeout(captureTimer); captureTimer = setTimeout(capture, 1800); });
  async function save(ref, transactionId = '') { const pricing = total(); return window.GOB_API.saveOrder({ ref, customer_name: fullName(), customer_phone: phone(), customer_email: get('[autocomplete="email"]'), items: items(), subtotal: pricing.value, total_amount: pricing.value, shipping: 0, discount: pricing.discount, grand_total: pricing.grand, payment_method: method() === 'cod' ? 'cod' : 'razorpay', transaction_id: transactionId || null, shipping_address: address(), address_details: addressDetails(), dog: dog(), coupon_code: pricing.coupon || null, loyalty_points_redeemed: pricing.points, packaging: method() === 'cod' ? 40 : 0, marketing_consent: marketingConsent(), checkout_policy_acknowledged: policyAcknowledged(), privacy_notice_version: noticeVersion() }); }
